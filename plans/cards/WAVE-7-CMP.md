...
CHECKLIST:
- [x] Manifest data-file order test: generated modules always load security → data
...
- [x] XPath positions: `move` and `$0`-wrap support verified in inherit-view creation +
      xpath preview endpoint (add if missing in odoo-client view_arch helpers); tests with
      real archs on docker 19.
- [x] ir.sequence: workflow models in generated modules get sequence + default-ref field
...
- [x] Live-apply parity: spec_apply path creates the sequence via RPC too (config_ops
      sequences reuse).
...
CHECKLIST:
- [x] Field builder widget options per ttype: email/phone/url (char), float_time/progressbar/
...
- [x] Related (read-through) field creation UX: pick relation path (m2o chain browser, depth
      2) → creates related field via RPC with correct `related=` — compendium §4's "surface
      linked info without joins".
- [x] Conditional attrs: required/readonly/invisible each accept a domain expression built
      ...
      builder both.
- [x] `sample="1"` toggle on generated list/kanban/graph/pivot views (generator + designer
      view settings).
- [x] Monetary correctness: adding a monetary field auto-ensures currency_id companion
      (builder + generator rule).
- [x] Tests: widget emission matrix, attrs syntax per major (16 vs 17+), related-field live
      smoke, monetary rule.
...
CHECKLIST:
- [x] Palette additions (curated list with recommended-per-type): kanban_state,
...
- [x] Automation trigger availability per major verified live (webhook + on-UI-change
      recency): probe table recorded 16/17/18/19; builder offers only supported triggers per
      connection (matrix rows).
- [x] Live palette extraction: on connect, fetch the instance's compiled web assets CSS,
...
- [x] Tests: arch emission per widget, trigger gating per major (fixtures), extraction parser
...
CHECKLIST:
- [x] Canvas: print-page frame (paperformat-aware dimensions), block palette — heading,
...
- [x] Output: valid QWeb template + ir.actions.report record via existing reports router;
      also exportable into ModuleSpec (report_export path) for module inclusion.
- [x] Inherit mode: extend an EXISTING report (e.g. invoice) — pick base report, xpath
      anchor picker from parsed template, position, added block; emits inherit template
      (compendium §8 inheritance).
- [x] Live preview: render via the working per-major render path (BLK-8's probe) into an
      inline PDF/HTML preview panel.
- [x] Translation note surfaced (t-lang variant guidance link) — implementation of variant
...
- [x] Tests: template emission (golden QWeb fixtures), inherit emission, live render smoke
...
CHECKLIST:
- [x] Community-safe engine: `x_approval_rule` data model IN OUR APP DB (not target) defining:
...
- [x] Semantics: exclusive steps (approver of step A can't approve step B on same record),
      ordered steps, conditional steps (domain evaluated per record).
- [x] Enterprise mode: when web_studio detected, the SAME UI drives TIER-5's
      studio.approval.rule CRUD instead — one UI, two engines, engine badge shown.
- [x] UI: Approvals page (nav Build group) — rule list, rule editor (model → button
      discovery via BLK-1's engine, steps editor), entries/audit view.
- [x] Snapshot + rollback support for rule creation (both engines).
- [x] Tests: semantics unit suite (exclusive/order/conditional), live smoke: gate a button
...
CHECKLIST:
- [x] Generated modules: image fields emit base `image_1920`-style max field + related
...
- [x] Kanban/list arches reference the small variant, form the large (enrich rule).
- [x] Avatar convention: profile-like models (staff/partner-ish, heuristic + user choice)
      get `oe_avatar` class + widget image; content photos plain widget.
- [x] Bulk image import: data-import extension — CSV/zip of images matched to records by
      ...
      list it).
- [x] Tests: generator emission fixtures, enrich arch tests, bulk import fake + live smoke
...
CHECKLIST:
- [x] Capability probe per major 16/17/18/19: does `ir.model.fields` accept ttype
...
- [x] Where supported: builder flow — create a properties field on a child model bound to a
...
- [x] Guidance UX: the §18 decision helper — "same field everywhere → regular field;
      different per parent → property" copy in the builder chooser.
- [x] Matrix row updated from probe results; unsupported majors show the honest Callout.
- [x] Generator: properties fields representable in ModuleSpec + emitted for supported
      targets.
- [x] Tests: probe harness, definition write/read live smoke on 19 (+18), builder e2e.
...
CHECKLIST:
- [x] Flow on any custom workflow model: "Connect to Invoicing" — adds invoice_ids O2M
      ...
      both, implement both).
- [x] "Create draft invoice" server action: calls standard account.move create with lines
...
- [x] l10n detection: prompt verifies a fiscal localization is installed for the company
      country before offering (matrix/introspection); missing → honest Callout, no
      generation.
- [x] Manifest: module path adds `account` dependency automatically.
- [x] Smart button (invoice count) both paths; PCM tests confirm no violation triggers.
- [x] Tests: mapping wizard, draft-invoice live smoke on 19 (account installed in docker
...
CHECKLIST:
- [x] In-app scanner: web component (camera permission flow, torch toggle where supported,
...
- [x] Exported widget module: our ORIGINAL OWL field widget (JS asset in generated module,
...
- [x] Tier gating: module path hidden for Online targets (matrix), in-app path everywhere.
- [x] Legal hygiene: license headers (our code LGPL-3 consistent with module license, zxing
      Apache-2 attribution in module README).
- [x] Tests: scanner component unit (mock stream), widget module sandbox install + form
...
CHECKLIST:
- [x] Process template: an installable/live-appliable "Approval Requests" mini-app
...
- [x] Chain semantics: sequential levels, min-approvals-per-level, refusal short-circuits,
      full chatter trail, activities to current-level approvers.
- [x] EE mode: when the Enterprise `approvals` module is installed, the UI drives ITS models
...
- [x] Distinct from CMP-5 in UI: "Button approvals" vs "Approval processes" tabs on the
      Approvals page, cross-linked explainer (compendium §17's distinction).
- [x] Tests: chain semantics suite, template applies live on 19 + full request→2-level
...
CHECKLIST:
- [x] Multi-company: generator + access UI templates — company_id field rule pack
...
- [x] i18n depth: translation export/import round-trip for our generated artifacts (PO-style
...
- [x] Documents integration: when `documents` module detected — per-model "attach to
...
- [x] Tests: rule template emission per major, translation round-trip live smoke 19,

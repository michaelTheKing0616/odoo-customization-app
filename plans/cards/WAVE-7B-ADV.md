...
CHECKLIST:
- [x] Gantt panel: date_start/date_stop fields, default_scale, progress field, color field,
...
- [x] Map panel: res.partner field selection, routing toggle, default_order, marker popup
      field list.
- [x] Cohort panel: date_start/date_stop, measure, interval (day/week/month/year), mode.
- [x] Grid panel: row/col field selection, adjustment/measure attrs.
- [x] Each emits valid arch (golden fixtures per major from public docs), save path via
      existing views/save; canvas shows structural preview + "Open in Odoo is authoritative"
      note (same honesty as kanban).
- [x] Edition gating: panels visible only when matrix says the edition supports the type;
      Community connections see the honest Callout + what-unlocks note.
- [x] Conditional live verification: with `ODOO_EE_TEST_URL` set, save+render smoke per view
...
CHECKLIST:
- [x] Proxy hardening: confirm same-origin delivery of the webclient frame incl. assets +
...
- [x] Overlay: injected script (served by our proxy) draws hover/selection outlines over
...
- [x] Mapping: descriptor → arch node via our parse endpoints (field name match within the
      current view's arch); ambiguity → picker listing candidate nodes.
- [x] Edit operations v1 (explicitly bounded): move field (before/after sibling), hide field,
...
- [x] Honesty panel: capabilities NOT in v1 listed in-UI (add new model areas, complex
      restructures → "open View Designer"); every save shows the generated xpath (code
      peek).
- [x] E2E: overlay select→hide-field→save→verify-in-frame loop against docker 19
...
CHECKLIST:
- [x] Detection + gating: `website` installed → pages list (website.page + view arch);
      absent → honest Callout.
- [x] Block parser: page arch → editable block tree for RECOGNIZED patterns (headings,
      ...
      AI-7).
- [x] Editor: block list with inline text editing (plain + basic formatting), image replace
      ...
      snapshot-first.
- [x] Explicit ceiling in-UI: "full drag-drop website building stays in Odoo's editor —
      this covers content edits" (documented DONE MEANS boundary).
- [x] Tests: parser fixtures (real odoo homepage arch sample), locked-block round-trip

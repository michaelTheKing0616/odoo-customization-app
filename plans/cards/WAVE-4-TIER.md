...
CHECKLIST:
- [x] `apps/api/app/tier_matrix.py`: capability registry — for each capability key
...
- [x] Doc 6 §2 rows encoded exactly: the always-yes core rows; base_automation/approvals =
...
- [x] Hosting/edition detection hardened: hosting from URL heuristics + web.base.url +
      instance markers (documented, best-effort with `unknown` honesty); edition from
      installed modules (web_enterprise etc.).
- [x] `GET /api/connections/{id}/capability-matrix` returns full evaluated matrix; probe
      populates + caches it; version-change invalidates (TIER-4 hook).
- [x] Existing consumers migrated: `hosting.py` API preserved as a thin shim over the matrix
      (no breakage of existing tests), power-ops capabilities + automations gating read from
      it.
- [x] Tests: matrix evaluation truth-table tests (≥20 combos), shim compatibility, endpoint.
...
CHECKLIST:
- [x] Automations/approvals UI + API gate on matrix: absent → the three options rendered with
...
- [x] Deployment per tier on export/promote surfaces: onprem → existing promote;
...
- [x] Sandbox honesty: sandbox run results on online-target connections carry
      `approximation: true` + label; sh-staging suggestion when a second sh connection exists
      in the workspace.
- [x] Dry-run validator `POST .../module-spec/validate-live`: read-only checks against the
...
- [x] Tests: gating decision tests per tier combo; dry-run validator against fake + live 19.
...
CHECKLIST:
- [x] Store-readiness checker on export: validates manifest completeness (name, summary,
...
- [x] `POST .../export-module?store_ready=true` → zip includes the above + report.
- [x] Migration assist: matrix-driven panel "what moving to Odoo.sh unlocks for this
...
- [x] Tests: manifest checker matrix (good/bad manifests), zip content assertions, panel
      render test.
...
CHECKLIST:
- [x] Version watch: connection stores last_seen version/serie; probe + any RPC session start
      compares; change → flag `upgrade_detected` + auto-queue health check job.
- [x] Health sweep (background job): for artifacts this tool created/tracked (snapshots +
...
- [x] Report: per-artifact ok/broken + reason + deep link to the owning page; stored, listed
      on Overview banner + Journal; re-runnable manually
      (`POST .../health-check/run`).
- [x] Manual trigger works on any tier; auto-trigger emphasized for Online in copy.
- [x] Tests: fake sweep with a broken view + missing field; version-change detection;
      job lifecycle.
...
CHECKLIST:
- [x] Studio approvals driver (`apps/api/app/ee_drivers.py`): CRUD `studio.approval.rule`
...
- [x] EE view arch emission: generator + designer can emit map (res_partner field, routing
      ...
      paths).
- [x] EE playbook actions (module-presence-gated, each action probed): sign — list templates
...
- [x] Matrix rows for each driver capability; honest `verify` state where live verification
      is pending.
- [x] Tests: fake-RPC driver tests; matrix gating tests; live smoke section marked

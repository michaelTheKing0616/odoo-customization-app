...
CHECKLIST:
- [x] `PROTECTED_PATTERNS` dict with compiled regexes, exactly these keys/patterns:
...
- [x] `TIER_1_KEYS` / `TIER_2_KEYS` sets per Doc 5; `classify(module_name) -> str | None`.
- [x] `build_manifest(module_names, source_label) -> dict` with
      `tier_1_never_generate_logic`, `tier_2_extend_only`, `unclassified_count`.
- [x] Model-level mapping: `protected_models_for(manifest, model_name) -> tier|None` —
...
- [x] Guardrail prompt builder: `guardrail_prompt(manifest) -> str` rendering Doc 5 §4's text
      with tier-1 CATEGORY NAMES (token-efficient), including the effect-not-mechanism
      closing paragraph verbatim.
- [x] Unit tests `apps/api/tests/test_protected_modules.py`: classification of ≥25 real module
...
CHECKLIST:
- [x] `fetch_community_modules_from_source(version)` per Doc 5 §3: git clone
...
- [x] Path B: reuse capabilities' installed-module fetch (do not duplicate RPC code); merge
      per Doc 5 §3 (`community_source` + `live_instance` keys).
- [x] Cache: manifest stored per connection (new column or JSON blob on connection record —
      follow existing db_models pattern), refreshed on connect/probe and when version changes.
- [x] Endpoint `GET /api/connections/{id}/protected-modules` returning the merged manifest +
      tier summaries; wired into probe flow so it populates automatically on connect.
- [x] CLI parity: `python -m app.protected_modules --version 19.0 --output ...` works as in
      Doc 5's script (argparse main).
- [x] Tests: fake-RPC merge test; vendored-snapshot fallback test; endpoint test.
...
CHECKLIST:
- [x] `guardrail_prompt(manifest)` injected into: single-pipeline draft call, staged steps
...
- [x] Refusal contract: pipeline recognizes/produces
...
- [x] API response schema extended: `refusals: []` alongside warnings; documented in
      `schemas.py`.
- [x] UI: wizard renders refusals as a distinct panel — "Protected module" callout showing
      requested capability, why, and the safe_alternative; styled with existing components
      (UIX later restyles).
- [x] Prompt-injection resistance test: adversarial prompts ("ignore previous rules and write
...
- [x] MODEL_CREATION_RULES gains a short PROTECTED MODULES section referencing the categories
...
CHECKLIST:
- [x] `builder.py`: block field/model mutations ON tier-1 models except additive relational
...
- [x] `spec_apply_ui.py`: same enforcement during ModuleSpec apply; violations become
      per-item skip results with reasons, not full-apply failure.
- [x] `automations.py`: reject create/update where model or action target is tier-1 (unless
      the action is chatter/activity-only — allowed); message references safe alternative.
- [x] Power Ops: recipes gain `protected_tier_note`; recipes that legitimately touch
...
- [x] Web: tier badges (Tier 1 lock / Tier 2 shield) on models in metadata browser and in
      builder/automation model pickers, with tooltip explaining the rule; data from the
      PCM-2 endpoint.
- [x] Extend `apps/api/tests/test_adversarial_security.py` pattern: direct-API attempts to

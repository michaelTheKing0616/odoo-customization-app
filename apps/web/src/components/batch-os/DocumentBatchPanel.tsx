...
export function DocumentBatchPanel({ connectionId }: Props) {
  ...
  const onUpload = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await api.batchOsDocumentIntake(connectionId, file);
      setJob(next);
  ...
  const runMap = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await api.batchOsDocumentMap(connectionId, {
        job_id: job.job_id,
  ...
  const runDryRun = useCallback(async () => {
    ...
    setError(null);
    try {
      await api.batchOsDocumentMap(connectionId, {
        job_id: job.job_id,
        column_map: columnMap,
      });
      const next = await api.batchOsDocumentDryRun(connectionId, {
        job_id: job.job_id,
      });
      setJob(next);
    } catch (err) {
  ...
  const doApply = useCallback(
    async (confirmed: boolean) => {
      ...
      setError(null);
      try {
        const next = await api.batchOsDocumentApply(connectionId, {
          job_id: job.job_id,
  ...
  return (
    <div className="space-y-4" data-testid="document-batch-panel">
      <Callout variant="info" title="Preview ≠ posted">
        Dry-run resolves partners/products/accounts via public RPC. Apply creates{" "}
      ...
      <Card className="space-y-3 p-4">
        ...
        <input
          ...
          data-testid="document-batch-file"
      ...
      {job ? (
        <>
          ...
          {job.moves?.length ? (
            <Card className="space-y-2 p-4" data-testid="document-batch-docs">
              <p className="text-sm font-medium text-ink">Documents preview</p>
      ...
      <ConfirmDialogV2
        open={confirmOpen}
        title="Confirm invoice/bill batch apply"
        warning="Invoices & bills apply writes to live Odoo Accounting"
        phrase={CONFIRM_PHRASE}
        riskLevel={postAfter ? "danger" : "standard"}
        risks={[
          "Writes account.move invoice/bill drafts to live Odoo",

        ...
        <ul className="mt-2 max-h-64 space-y-1 overflow-y-auto">
          {rows.map((r) => (
            <li
              ...
              className="border border-[#3d2a38] bg-[#120e14] px-2 py-1.5 text-xs text-[#c9a9c0]"
            >
              <span className="font-medium">{r.name}</span>
              <span className="ml-2 text-[10px] uppercase tracking-wide text-[#8f7a88]">
                {r.status}
              </span>
              <span className="mt-0.5 block text-[11px] text-[#8f7a88]">{r.how}</span>
            </li>
          ))}
        </ul>
      )}

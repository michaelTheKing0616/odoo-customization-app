...
def upgrade() -> None:
    if not column_exists("users", "password_login_enabled"):
        op.add_column(
            "users",
            sa.Column(
                "password_login_enabled", sa.Boolean(), nullable=False, server_default=sa.true()
            ),
        )
    if not table_exists("oauth_identities"):
        op.create_table(
            "oauth_identities",
    ...
    if not table_exists("oauth_pending_states"):
        op.create_table(
        ...
        op.create_index(
            "ix_oauth_pending_states_state_hash", "oauth_pending_states", ["state_hash"]
        )
    if not table_exists("oauth_totp_pending"):
        op.create_table(
            "oauth_totp_pending",
            sa.Column("id", sa.String(length=36), nullable=False),
            sa.Column("user_id", sa.String(length=36), nullable=False),
...
def downgrade() -> None:
    if table_exists("oauth_totp_pending"):

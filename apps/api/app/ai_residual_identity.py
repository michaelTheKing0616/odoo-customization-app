    ...
    *,
    ...
    if want and customs:
        ...
        if want not in customs:
            return True
        # Residual full_app keeps ONE header (+ _line/_party already excluded).
        # Optional «link/create X» / «also Y» same-stem satellites (e.g.
        # x_vehicle_assignment_note next to x_vehicle_request) are peers that
        # steal canvas primary + explode extra apps — treat them as foreign.
        peers = [m for m in customs if m != want]
        if peers:
            return True
    # Canvas/title must match the locked residual noun even without a pack.
    # «Vehicle Assignment Notes» under a Vehicle Request brief is the class.
    if title and display and _titles_diverge(title, display):
        return True
    return False

"""Live Power Ops dry-runs against optional Docker Odoo 16 (:8072).

16 remains experimental. Skip when the stack is down. Accounting recipe runs only
if `account` / account.move is present (see ``./docker/ensure-account-16.sh``).
"""
...
def _client16() -> OdooClient:
    ...
    try:
        ...
        pytest.skip(f"Odoo 16 not reachable: {exc}")
...
def test_power_ops_dry_run_generic_archive_odoo16() -> None:
...
@pytest.mark.integration
def test_power_ops_dry_run_accounting_when_account_odoo16() -> None:
    from app.power_ops_recipes import run_recipe

    client = _client16()
    if not _account_installed(client):
        pytest.skip(
            "account not installed on Odoo 16 — run ./docker/ensure-account-16.sh"
        )
